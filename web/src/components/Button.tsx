  ...
  return (
    <button
      ...
      className={`
			...
			${
        theme === "outline" &&
        "bg-transparent text-primary border border-primary hover:text-primary-hover hover:border-primary-hover"
      }
			${
        theme === "outline-secondary" &&
        "bg-transparent text-gray-500 border border-gray-500 hover:bg-gray-500 hover:text-white"
      }
			${theme === "secondary" && "bg-gray-100 text-gray-500 hover:bg-gray-200"}
      ${theme === "disabled" && "bg-gray-200 text-gray-400 cursor-not-allowed"}
			font-bold rounded-2xl cursor-pointer transition  inset-0 flex items-center justify-center
			${className}
			`}
      disabled={isLoading || theme === "disabled"}
      {...props}
    >
      {isLoading && (
